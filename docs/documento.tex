...
El primero tipo de figura (\ref{fig:figura1}, \ref{fig:figura2}, \ref{fig:figura3}, \ref{fig:figura4}, \ref{fig:figura5}) permite comparar las cuatro métricas usadas (Accuracy, F1 Score, Precision y Recall) entre los distintos modelos entrenados sobre un conjunto de datos en concreto, habiendo cinco figuras para cinco conjuntos de datos distintos, en estas gráficas se comparan cuatro estados distintos, CNN sin aumento, para comprobar su robustez ante datos similares, CNN con aumento para observar si es capaz de adaptarse correctamente a datos externos al dataset o datos modificados visualmente, también se observan los resultados del KNN sin aumento de datos para observar si el ajuste de pesos se ha realizado correctamente y KNN con aumento de datos para observar su adaptabilidad a datos anormales en el dataset.
El segundo tipo de figura (\ref{fig:figura6}, \ref{fig:figura7}, \ref{fig:figura8}, \ref{fig:figura9}) muestra cómo varía una métrica para todos los modelos que se encuentran para obtener una comparación entorno a las métricas mientras que el primero lo hace entorno a los modelos y experimentos.

Se pueden apreciar una serie de hechos que se ven reflejados en las gráficas:
